...
protocol AddressRemoteDataSource {
    func fetchAddresses(accessToken: String) async throws -> [CustomerAddress]
    func createAddress(
        accessToken: String,
        address: StorefrontMailingAddressInput
    ...
    func setDefaultAddress(accessToken: String, addressId: String) async throws
        -> CustomerAddress?
}
...
final class AddressGraphQLRemoteDataSource: AddressRemoteDataSource {
    ...
    func fetchAddresses(accessToken: String) async throws -> [CustomerAddress] {
        let request = GraphQLRequest(
            query: StorefrontAddressQueries.fetchAddresses,
            variables: StorefrontFetchAddressesVariables(
                customerAccessToken: accessToken
            ),
            operationName: "FetchAddresses"
        )
        let response: StorefrontFetchAddressesResponse =
            try await client.request(request)
        let defaultId = response.customer?.defaultAddress?.id
        return response.customer?.addresses.nodes.map {
            $0.toDomain(isDefault: $0.id == defaultId)
        } ?? []
    }
    ...
    func createAddress(
        ...
        let request = GraphQLRequest(
            ...
            variables: StorefrontCreateAddressVariables(
                customerAccessToken: accessToken,
                address: address
            ),
            operationName: "CustomerAddressCreate"
        )
        let response: StorefrontCreateAddressResponse =
            try await client.request(request)
    ...
    func updateAddress(
        ...
        let request = GraphQLRequest(
            ...
            variables: StorefrontUpdateAddressVariables(
                customerAccessToken: accessToken,
                id: id,
                address: address
            ),
        ...
        let response: StorefrontUpdateAddressResponse =
            try await client.request(request)
    ...
    func deleteAddress(accessToken: String, id: String) async throws -> String {
        let request = GraphQLRequest(
            ...
            variables: StorefrontDeleteAddressVariables(
                customerAccessToken: accessToken,
                id: id
            ),
            operationName: "CustomerAddressDelete"
        )
        let response: StorefrontDeleteAddressResponse =
            try await client.request(request)
        ...
        -> CustomerAddress?
    {
        let request = GraphQLRequest(
            ...
            variables: StorefrontSetDefaultAddressVariables(
                customerAccessToken: accessToken,
                addressId: addressId
            ),
            operationName: "CustomerDefaultAddressUpdate"
        )
        let response: StorefrontSetDefaultAddressResponse =
            try await client.request(request)
